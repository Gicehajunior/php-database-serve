...
module.exports = class Database {
  ...
  execute_commands() {
    if (this.args.length > 0) {
      ...
      console.log(
        `No arguments provided. Please see Help, https://github.com/Gicehajunior/php-database-serve.`
      );
    }
  }
  ...
  inquire_user(database_name) {
    ...
    if (database_name == "") {
      ...
      query = {
        type: "confirm",
        name: "database_name",
        message: "Ready to continue with the existing database name? ",
        default: false,
      };
  ...
  overwrite_env_file(dotenv_filepath) {
    ...
    fs.readFile(`${dotenv_path}`, "utf8", (error, data) => {
      if (error) {
        ...
        let env_replacement = data.replace(
          `${this.database_name}`,
          this.new_database_name
        );
  ...
  prompt_user(query) {
    inquirer.prompt([query]).then((answer) => {
      if (
        query.message.includes(
          "Ready to continue with the existing database name? "
        )
      ) {
        ...
        query.message.includes("Do you want to migrate database tables? ")
      ) {
        this.migrate_database_tables();
      }
    });
  ...
  run_search_for_sql_dump_file(files_path) {
    ...
    files_array.forEach((non_pathed_file) => {
      ...
      let files_extension = fname.slice(
